...
RightTopFrame::RightTopFrame(CWnd* pWnd, CRect rect)
{
	...
	// создаЄм контекст устройства (нашего текущего фрейма)
	CClientDC dc(this);
	...
	// битового образа дл€ использовани€
	// его в качестве буфера изображени€

	m_memDC.CreateCompatibleDC(&dc);
	m_bmp.CreateCompatibleBitmap(&dc, maxX, maxY);

	m_memDC.SelectObject(&m_bmp);
	m_bkbrush.CreateStockObject(WHITE_BRUSH); // дл€ стирани€ фона (бела€ кисть)
...
afx_msg BOOL RightTopFrame::OnEraseBkgnd(CDC* pDC)
{
	...
	// –исуем координатные оси и отметки важные
	// вертикально:
	for (
